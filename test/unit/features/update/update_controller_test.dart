...
class FakeUpdateService extends PlayStoreUpdateService {
  FakeUpdateService({
    required this.result,
          ...
          isSupportedBuild: (PackageInfo _) => true,
...
void main() {
  ...
  setUp(() {
    ...
    // runSilentCheck 只读不写，但 openStorePage/dismissCurrentVersion 会写
  ...
  test('openStorePage consumes dialog opportunity for the rest of the session', () async {
    final service = FakeUpdateService(
      result: const UpdateCheckResult.available(2026061501),
    );
    final tasks = TaskController();
    ...
    await controller.openStorePage();
  ...
  test('dismissCurrentVersion suppresses dialog for the same available version', () async {
  ...
  test('dialog stays suppressed inside cooldown window when version changes', () async {
    ...
    service.result = const UpdateCheckResult.available(newVersion);
